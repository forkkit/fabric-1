...
func (entity *Terminator) LoadValues(_ boltz.CrudStore, bucket *boltz.TypedBucket) {
	...
	entity.Identity = bucket.GetString(FieldTerminatorIdentity)
	entity.IdentitySecret = bucket.GetString(FieldTerminatorIdentitySecret)
...
func (entity *Terminator) SetValues(ctx *boltz.PersistContext) {
	...
	ctx.SetStringP(FieldTerminatorIdentity, entity.Identity)
	ctx.SetStringP(FieldTerminatorIdentitySecret, entity.IdentitySecret)
...
func (store *terminatorStoreImpl) validateIdentitySecret(ctx *boltz.PersistContext, entity *Terminator) {
	...
	for _, terminatorId := range terminatorIds {
		if terminatorId != ctx.Id {
			if terminator, _ := store.LoadOneById(ctx.Bucket.Tx(), terminatorId); terminator != nil {
				if terminator.Identity != nil && identity == *terminator.Identity {
	...
	for _, terminator := range identityTerminators {
		if entity.IdentitySecret == nil && terminator.IdentitySecret == nil {
			continue
		}

		if (entity.IdentitySecret == nil && terminator.IdentitySecret != nil) ||
			(entity.IdentitySecret != nil && terminator.IdentitySecret == nil) ||
			(*entity.IdentitySecret != *terminator.IdentitySecret) {
